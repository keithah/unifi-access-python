...
from .models import (
    User, Visitor, AccessPolicy, Schedule, HolidayGroup,
    NFCCard, PINCode, TouchPass, Door, DoorGroup, Device,
    AccessEvent, SystemLog, CredentialType, UserRole, DeviceType
)
...
class UniFiAccessClient:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                async with self.session.request(
                    ...
                    if response.status == 200 or response.status == 201:
                        ...
                        raise AuthenticationError(
                            response_data.get('message', 'Authentication failed - invalid token'),
                            status_code=response.status,
                            response_data=response_data
                        )
                ...
                if attempt == self.max_retries:
                    raise ConnectionError(f"Request failed after {self.max_retries + 1} attempts: {e}")
    ...
    def _parse_user(self, data: Dict[str, Any]) -> User:
        ...
        return User(
            ...
            touch_passes=[self._parse_touch_pass(tp) for tp in data.get('touch_passes', [])],
    ...
    async def add_visitor_pin(self, visitor_id: str, pin: str) -> bool:
        ...
        """
        data = {"pin_code": pin}
        response = await self._request("PUT", f"/visitors/{visitor_id}/pin_codes", data=data)
        return response.get('code') == 'SUCCESS'
    
    async def add_visitor_nfc_card(self, visitor_id: str, card_number: str, facility_code: Optional[str] = None) -> NFCCard:
        """
        ...
        response = await self._request("POST", f"/visitors/{visitor_id}/nfc-cards", data=data)
    ...
    def _parse_visitor(self, data: Dict[str, Any]) -> Visitor:
        ...
        start_date = datetime.fromtimestamp(data['start_time']) if data.get('start_time') else datetime.utcnow()
        end_date = datetime.fromtimestamp(data['end_time']) if data.get('end_time') else datetime.utcnow()
    ...
    async def unlock_door(self, door_id: str, duration: int = 10) -> bool:
        ...
        """
        raise ResourceNotFoundError("Door unlock functionality not available in developer API")
    ...
    async def lock_door(self, door_id: str) -> bool:
        ...
        """
        raise ResourceNotFoundError("Door lock functionality not available in developer API")
    ...
    async def get_door_groups(self, limit: int = 100, offset: int = 0) -> List[DoorGroup]:
        """
    ...
    async def create_door_group(self, name: str, description: Optional[str] = None, door_ids: Optional[List[str]] = None) -> DoorGroup:
        """
    ...
    def _parse_device(self, data: Dict[str, Any]) -> Device:
        ...
        return Device(
            ...
            type=device_type,
            mac_address=data.get('id', ''),  # Use device ID as MAC since real MAC not provided
            ...
            door_id=data.get('location_id'),  # For door devices, location_id is the door
            created_at=None,  # Not provided in API
            updated_at=None   # Not provided in API
        )
    ...
    async def get_access_policies(self, limit: int = 100, offset: int = 0) -> List[AccessPolicy]:
        """
    ...
    async def get_holiday_groups(self, limit: int = 100, offset: int = 0) -> List[HolidayGroup]:
        """
        ...
        """
        raise ResourceNotFoundError("Holiday groups endpoint not available in developer API")
    ...
    async def get_access_events(
        ...
        """
        raise ResourceNotFoundError("Access events endpoint not available in developer API")
    ...
    async def get_system_logs(
        ...
        """
        raise ResourceNotFoundError("System logs endpoint not available in developer API")
    ...
    def create_websocket_client(self) -> UniFiAccessWebSocket:
        ...
        """
        raise ResourceNotFoundError("WebSocket functionality not available in developer API")