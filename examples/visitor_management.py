...
async def create_temporary_visitor(client):
    ...
    try:
        ...
        print(
            f"   Access period: {start_date.strftime('%Y-%m-%d %H:%M')} to {end_date.strftime('%Y-%m-%d %H:%M')}"
        )
...
async def list_all_visitors(client):
    ...
    try:
        ...
        for visitor in active_visitors[:5]:
            ...
            print(
                f"        📅 Access: {visitor.start_date.strftime('%m/%d')} - {visitor.end_date.strftime('%m/%d')}"
            )
            print()
    ...
    async with UniFiAccessClient(
        ...
        if visitor:
            ...
            if updated_visitor:
                ...
                print(
                    f"   Name: {updated_visitor.first_name} {updated_visitor.last_name}"
                )
